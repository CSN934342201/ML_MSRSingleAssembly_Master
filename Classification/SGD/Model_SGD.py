...
def TrainSaveMLmodel(Dir_TrainDataSample,Dir_LoadedScaler,Dir_MLModel):
    ...
    sgd = linear_model.SGDClassifier(random_state=43)
    param_grid = {'loss':['hinge','log','modified_huber','square_hinge','perception'],'penalty':['l2','l1','elasticnet'],'alpha':np.arange(1e-3,1e-1,0.0005),'l1_ratio':np.arange(0.01,1,0.005)}
    gs = GridSearchCV(sgd,param_grid,cv=5,scoring='f1')
    ...
    # Save the ML model
    sgd = linear_model.SGDClassifier(random_state=43,alpha=gs.best_params_['alpha'],loss=gs.best_params_['loss'],penalty=gs.best_params_['penalty'],l1_ratio=gs.best_params_['l1_ratio'])
    sgd.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(sgd, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('SGD classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('SGD_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_SGD.pkl'
