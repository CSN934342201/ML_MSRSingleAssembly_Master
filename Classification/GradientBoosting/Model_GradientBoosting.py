...
def TrainSaveMLmodel(Dir_TrainDataSample, Dir_LoadedScaler, Dir_MLModel):
    ...
    learning_rate = np.arange(0.1,1,0.005)
        ...
        for hp2 in n_estimators:
            for hp3 in max_depth:
                for hp4 in max_feature:
                    ...
                    # Instantialization  
                    gb_clf = GradientBoostingClassifier(random_state=43,learning_rate=hp1,n_estimators=hp2,max_depth=hp3,max_features=hp4)

                    # 5-Fold cross validation
                    f1.append(cross_val_score(gb_clf, Data_TrainVal, Target_TrainVal, cv=5, scoring='f1').mean())
                    rocauc.append(cross_val_score(gb_clf, Data_TrainVal, Target_TrainVal, cv=5, scoring='roc_auc').mean())
    ...
    gb_clf = GradientBoostingClassifier(random_state=43,learning_rate=ParamOptimized[0],n_estimators=ParamOptimized[1],max_depth=ParamOptimized[2],max_features=ParamOptimized[3])
    gb_clf.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(gb_clf, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('GB classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('GB_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_GradientBoosting.pkl'
