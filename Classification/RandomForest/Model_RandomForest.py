...
def TrainSaveMLmodel(Dir_TrainDataSample, Dir_LoadedScaler, Dir_MLModel):
    ...
    for hp1 in max_feature:
        for hp2 in n_estimators:
            for hp3 in max_depth:
                ...
                # Instantiation  
                rf = RandomForestClassifier(max_depth=hp3, n_estimators=hp2, max_features=hp1, random_state=43)
                # 5-Fold cross validation
                f1.append(cross_val_score(rf, Data_TrainVal, Target_TrainVal, cv=5, scoring='f1').mean())
                rocauc.append(cross_val_score(rf, Data_TrainVal, Target_TrainVal, cv=5, scoring='roc_auc').mean())
    ...
    rf = RandomForestClassifier(max_depth=ParamOptimized[2], n_estimators=ParamOptimized[1], max_features=ParamOptimized[0], random_state=43)
    rf.fit(Data_TrainVal, Target_TrainVal)
    joblib.dump(rf, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('RandomForest classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('RandomForest_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_RandomForest.pkl'
