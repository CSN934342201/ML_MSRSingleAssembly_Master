...
def TrainSaveMLmodel(Dir_TrainDataSample, Dir_LoadedScaler, Dir_MLModel):
    ...
    for hp1 in learning_rate:
        for hp2 in max_depth:
            ...
            print('Hyperparameter in iteration: ',hp1,hp2)
            ParamList.append([hp1,hp2])

            # Instantialization  
            gbm = lgb.LGBMClassifier(objective='binary',max_depth=hp2,learning_rate=hp1,random_state=43)

            # 5-Fold cross validation
            f1.append(cross_val_score(gbm, Data_TrainVal, Target_TrainVal, cv=5, scoring='f1').mean())
            rocauc.append(cross_val_score(gbm, Data_TrainVal, Target_TrainVal, cv=5, scoring='roc_auc').mean())
    ...
    gbm = lgb.LGBMClassifier(objective='binary',max_depth=ParamOptimized[1],learning_rate=ParamOptimized[0],random_state=43)
    gbm.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(gbm, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('LGBM classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('LGBM_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_LightGBM.pkl'
