...
def TrainSaveMLmodel(Dir_TrainDataSample,Dir_LoadedScaler,Dir_MLModel):
    ...
    param_grid = {'n_neighbors':np.arange(3,10,1),'weights':['uniform','distance'],'algorithm':['ball_tree','kd_tree','brute','auto'],'leaf_size':np.arange(2,50,1),'p':np.arange(1,10,1)}
    ...
    # Save the ML model
    knn = sklearn.neighbors.KNeighborsClassifier(n_neighbors=gs.best_params_['n_neighbors'],weights=gs.best_params_['weights'],algorithm=gs.best_params_['algorithm'],leaf_size=gs.best_params_['leaf_size'],p=gs.best_params_['p'])
    knn.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(knn, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('KNN classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('KNN_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_KNN.pkl'
