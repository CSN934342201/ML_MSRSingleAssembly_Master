...
def TrainSaveMLmodel(Dir_TrainDataSample, Dir_LoadedScaler, Dir_MLModel):
    ...
    n_estimators = np.arange(70,300,10)
    ...
    for hp1 in n_estimators:
        for hp2 in max_depth:
            for hp3 in max_feature:
                ...
                # Instantialization  
                bagging = BaggingClassifier(random_state=43,n_estimators=hp1,base_estimator=DecisionTreeClassifier(max_depth=hp2,max_features=hp3,random_state=43))

                # 5-Fold cross validation
                f1.append(cross_val_score(bagging, Data_TrainVal, Target_TrainVal, cv=5, scoring='f1').mean())
                rocauc.append(cross_val_score(bagging, Data_TrainVal, Target_TrainVal, cv=5, scoring='roc_auc').mean())
    ...
    bagging = BaggingClassifier(random_state=43,n_estimators=ParamOptimized[0],base_estimator=DecisionTreeClassifier(max_depth=ParamOptimized[1],max_features=ParamOptimized[2],random_state=43))
    bagging.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(bagging, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('Bagging classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('Bagging_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_Bagging.pkl'
