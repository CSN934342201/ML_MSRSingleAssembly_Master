...
def TrainSaveMLmodel(Dir_TrainDataSample, Dir_LoadedScaler, Dir_MLModel):
    ...
    n_estimators = np.arange(70,300,1)
    ...
    for hp1 in n_estimators:
        for hp2 in max_depth:
            for hp3 in max_feature:
                ...
                # Instantialization  
                extra_tree = ExtraTreesClassifier(random_state=43,n_estimators=hp1,max_depth=hp2,max_features=hp3)

                # 5-Fold cross validation
                f1.append(cross_val_score(extra_tree, Data_TrainVal, Target_TrainVal, cv=5, scoring='f1').mean())
                rocauc.append(cross_val_score(extra_tree, Data_TrainVal, Target_TrainVal, cv=5, scoring='roc_auc').mean())
    ...
    extra_tree = ExtraTreesClassifier(random_state=43,n_estimators=ParamOptimized[0],max_depth=ParamOptimized[1],max_features=ParamOptimized[2])
    extra_tree.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(extra_tree, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('ET classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('ET_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_ExtraTree.pkl'
