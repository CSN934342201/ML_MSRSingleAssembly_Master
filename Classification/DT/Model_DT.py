...
def TrainSaveMLmodel(Dir_TrainDataSample,Dir_LoadedScaler,Dir_MLModel):
    ...
    dt = sklearn.tree.DecisionTreeClassifier(random_state=43)
    param_grid = {'max_depth':np.arange(2,30,1),'min_samples_split':np.arange(2,6,1),'min_samples_leaf':np.arange(1,6,1),'max_features':np.arange(2,17)}
    ...
    # Save the ML model
    dt = sklearn.tree.DecisionTreeClassifier(random_state=43,max_features=gs.best_params_['max_features'],max_depth=gs.best_params_['max_depth'],min_samples_split=gs.best_params_['min_samples_split'],min_samples_leaf=gs.best_params_['min_samples_leaf'])
    dt.fit(Data_TrainVal,Target_TrainVal)
    joblib.dump(dt, Dir_MLModel)
...
def EvaluateModel(TestDataSample_Target, predictions):
    ...
    plt.title('DT classifier',fontweight='bold',fontsize=20)
    plt.tight_layout()
    plt.savefig('DT_ConfusionMatrix',dpi=600)
    plt.show()
...
if __name__ == '__main__':
    ...
    Dir_MLModel = 'Your directory of Model_DT.pkl'
